...
| **SQL validation command** | `apps/backend/scripts/phase31-validation.sh integrity` (creado en Wave 0)                  |
...
- **Phase gate (entre Deploys 1↔2, 2↔3):** `apps/backend/scripts/phase31-validation.sh integrity` debe retornar exit 0 ANTES de avanzar
...
| ------------------ | ------------------------- | ---- | ----------- | ---------- | ---------------------------------------------------------------------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| 31-01-01           | 31-01 (Preflight)         | 0    | VAR-10      | —          | Preflight audit informativo no bloquea; output a 31-PREFLIGHT-AUDIT.md | smoke SQL    | `apps/backend/scripts/phase31-preflight-audit.sh`                                                                                                                                                           | ❌ W0       |
...
| 31-03-05           | 31-03                     | 2    | VAR-10      | SC#5       | Integridad referencial: 5 queries retornan 0                           | smoke SQL    | `apps/backend/scripts/phase31-validation.sh integrity` exit 0                                                                                                                                               | ❌ W0       |
...
- [ ] `apps/backend/scripts/phase31-preflight-audit.sh` — Bash + psql que ejecuta las 4 queries de auditoría D-01 (null_sku, sku_eq_codigo, sku_diff_codigo, sku_dupes) y guarda output en `.planning/phases/31-.../31-PREFLIGHT-AUDIT.md`. Non-blocking.
- [ ] `apps/backend/scripts/phase31-validation.sh` — Bash + psql con sub-comandos: `integrity` (5 queries SC#5), `pk-swap` (verifica sku como PK), `triggers` (verifica trg_update_articulo_unidades ENABLED), `unidades-sync` (verifica articulos.unidades sincronizado). Exit 0 si todos OK, exit 1 si cualquiera falla.
